...
public class RenderersPageViewImpl extends Composite implements RenderersPageView {
    ...
    private static final RenderersPageViewImplUiBinder UI_BINDER = GWT.create(RenderersPageViewImplUiBinder.class);

    @UiField
    FlowPanel mainPanel;

    @UiField
    FlowPanel renderersPanel;
    ...
    private void addRendererSwitcherToPanel(OutputConsoleRenderer renderer, boolean applicable) {
        ...
        switcher.addValueChangeHandler(event -> delegate.onApplicableRendererChanged(renderer, event.getValue()));
