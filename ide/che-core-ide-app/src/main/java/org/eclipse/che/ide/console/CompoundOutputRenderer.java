...
/**
 * Compound renderer allows to combine several different renderers. A text
 * provided will be treated as 'can be rendered' if at least one of nested renderers
 * detects it as 'can be rendered'
 * 
...
public class CompoundOutputRenderer implements OutputConsoleRenderer {
    ...
    /**
     * Constructs the compound renderer object from a number of specified output
     * renderers
     * 
     * @param renderers
     */
    ...
    public String render(String text) {
        Optional<OutputConsoleRenderer> optional = Stream.of(renderers).filter(renderer -> renderer.canRender(text))
                .findFirst();
